...
public interface DaoIrumae {
    ...
    @Query("SELECT * FROM irumae_table where id = :id LIMIT 1") //id 기준으로 1테이블만 검색
    Irumae getById(int id);

    @Query("SELECT * FROM irumae_table ORDER BY id DESC") //모든 테이블 id 순으로 가져오기
    List<Irumae> getAll();
